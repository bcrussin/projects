...
class Graph {
  ...
  Graph(int[] lines, float[] xBound, float[] yBound) {
    ...
    for(int i = 0; i < this.numLines; i++) {
      if(Arrays.asList(this.lines).contains(i)) {
        ...
        this.maxWidth[i] = maxWidth[i];
      }
    }
    
    this.dataPadX *= dataTextSize;
    this.dataPadY *= dataTextSize;
    for(int i = 0; i < this.numLines; i++) {
  ...
  void update() {
    if(autoZooming) {
      this.camX[0] += (this.zoomTargetX[0] - this.camX[0]) / autoZoomSpd;
      this.camX[1] += (this.zoomTargetX[1] - this.camX[1]) / autoZoomSpd;
      this.camY[0] += (this.zoomTargetY[0] - this.camY[0]) / autoZoomSpd;
      this.camY[1] += (this.zoomTargetY[1] - this.camY[1]) / autoZoomSpd;
      float zoomDist = 0.1;
    ...
    rect(this.xBound[0], this.yBound[1], this.graphWidth, this.graphHeight);
    
    if(this.graphMode) {
      //draw origin lines
      if(!this.originOnTop) drawOriginLines(false);
      ...
      if(this.lineHover != -1 && this.lineToggle[this.lineHover]) {
        stroke(hoverCol);
        ...
        //draw origin lines
      if(this.originOnTop) drawOriginLines(true);
    } else {
        //show data view
      if(this.dataCamX > this.dataTotalWidth) this.dataCamX = this.dataTotalWidth;
      if(this.dataCamY > this.dataTotalHeight) this.dataCamY = this.dataTotalHeight;
      if(this.dataCamX < 0) this.dataCamX = 0;
      if(this.dataCamY < 0) this.dataCamY = 0;
        
      textSize(dataTextSize);
      textAlign(LEFT, TOP);
      strokeWeight(2);
      
      float xpos = this.dataPadX - this.dataCamX;
      ...
      for(int i = 0; i < this.numLines; i++) {
        for(int j = 0; j < numPoints; j++) {
          float ypos = this.dataPadY + ((dataTextSize + 5) * j * 2) - this.dataCamY;
          if(round(this.data[i][j]) == this.data[i][j]) text(int(this.data[i][j]), this.xBound[0] + xpos + 10, this.yBound[0] + ypos);
          else text(str(this.data[i][j]), this.xBound[0] + xpos + 10, this.yBound[0] + ypos);
        }
        xpos += this.maxWidth[i];
      }
      
        
      fill(dataLabelBG);
      noStroke();
      rect(this.xBound[0], this.yBound[0], this.dataPadX, this.graphHeight);
      rect(this.xBound[0], this.yBound[0], this.graphWidth, this.dataPadY);
      stroke(#000000);
      
      xpos = this.dataPadX - this.dataCamX;
      
        //data labels (x-axis)
      for(int i = 0; i < this.numLines; i++) {
        if(xpos > this.dataPadX) line(this.xBound[0] + xpos, this.yBound[0], this.xBound[0] + xpos, height - this.yBound[1]);
        
        fill(colors[this.lines[i]]);
        text(labels[this.lines[i]], this.xBound[0] + xpos + 5, this.yBound[0] + (dataTextSize));
        if(this.xBound[0] + xpos > width - this.xBound[1]) break;
        xpos += (this.maxWidth[i]);
      }
      line(this.xBound[0] + xpos, this.yBound[0], this.xBound[0] + xpos, height - this.yBound[1]);
      ...
      for(int i = 0; i < numPoints; i++) {
        float ypos = this.dataPadY + ((dataTextSize + 5) * i * 2) - this.dataCamY;
        if(ypos > this.dataPadY) line(0, this.yBound[0] + ypos, width - this.xBound[1] + this.dataTotalWidth - (dataTextSize / 2), this.yBound[0] + ypos);
        text(dataInterval * i, this.xBound[0] + 10, this.yBound[0] + ypos);
        if(this.yBound[0] + ypos > height - this.yBound[1]) break;
      }
      
      fill(dataLabelBG);
      noStroke();
      rect(this.xBound[0], this.yBound[0], this.dataPadX, this.dataPadY);
      stroke(#000000);
      strokeWeight(5);
      line(this.xBound[0] + (this.dataPadX), this.yBound[0], this.xBound[0] + (this.dataPadX), height - this.yBound[1]);
      line(0, this.yBound[0] + (this.dataPadY), width - this.xBound[1], this.yBound[0] + (this.dataPadY));
    }  //end data view
  }
}
